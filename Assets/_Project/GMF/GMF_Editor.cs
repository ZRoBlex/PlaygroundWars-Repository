// ╔══════════════════════════════════════════════════════════╗
// ║  ARCHIVO: GMF_Editor.cs                                  ║
// ║  CARPETA: Assets/_Project/GameModeFramework/Editor/      ║
// ║                                                          ║
// ║  CLASES INCLUIDAS:                                       ║
// ║    • GMFEditorWindow (EditorWindow)                      ║
// ║                                                          ║
// ║  RESPONSABILIDAD:                                        ║
// ║    Herramienta de debug del framework en Unity Editor.   ║
// ║                                                          ║
// ║  CONFIGURACIÓN EN UNITY:                                 ║
// ║    Window → GameMode Framework → Debug Window            ║
// ║    Pulsar "Find" para conectar con la escena activa.     ║
// ╚══════════════════════════════════════════════════════════╝
...
namespace GMF.Editor
{
    public class GMFEditorWindow : EditorWindow
    {
        ...
        {
            if (_subbed) return;
            EventBus<GameStartedEvt>.Subscribe(e =>
                Log($"<color=lime>[START]</color> {e.ModeID} R{e.Round}"));
            EventBus<GameEndedEvt>.Subscribe(e =>
                Log($"<color=cyan>[END]</color> {e.ModeID} Winner=T{e.WinnerTeamID} ({e.Reason})"));
            EventBus<RoundStartedEvt>.Subscribe(e =>
                Log($"<color=#aaffaa>[ROUND {e.Round}]</color> {e.Duration:F0}s"));
            EventBus<RoundEndedEvt>.Subscribe(e =>
                Log($"<color=gold>[ROUND END]</color> T{e.WinnerTeamID} gana"));
            EventBus<ObjectiveInteractedEvt>.Subscribe(e =>
                Log($"<color=yellow>[OBJ]</color> '{e.ObjectiveID}' {e.Interaction} P{e.PlayerID}"));
            EventBus<ScoreChangedEvt>.Subscribe(e =>
                Log($"<color=#88ff88>[SCORE]</color> T{e.TeamID} +{e.Delta} = {e.NewTeamTotal}"));
            EventBus<PlayerJoinedTeamEvt>.Subscribe(e =>
                Log($"[TEAM] P{e.PlayerID} → {e.TeamName}"));
            _subbed = true;
        }
        ...
        private void OnGUI()
        {
            ...
            {
                _gm  = Object.FindFirstObjectByType<GameModeBase>();
                _mgr = Object.FindFirstObjectByType<GameModeManager>();
            }
        ...
        private void DrawObjectivesTab()
        {
            ...
            if (objs == null || objs.Count == 0)
            {
                EditorGUILayout.HelpBox(
                    "Sin objetivos. Los ObjectiveBase se registran en Start().\n" +
                    "Entra en Play Mode y asegura que hay GameModeBase en escena.",
                    MessageType.Info);
                return;
            }
            ...
            foreach (var obj in objs)
            {
                ...
                EditorGUILayout.LabelField($"Estado: {obj.State}");
                GUI.color = Color.white;
                EditorGUILayout.LabelField($"TeamID: {(obj.TeamID < 0 ? "Neutral" : $"T{obj.TeamID}")}");
